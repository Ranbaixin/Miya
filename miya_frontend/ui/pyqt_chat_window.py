...
class ChatWindow(QWidget):
    ...
    def _init_Layout(self):
        ...
        self.prompt.setStyleSheet(
            f"color:#fff;font:{fontsize}pt '{fontfam}';background:transparent;"
        )
    ...
    def _init_buttons(self):
        ...
        # 添加心智云图按钮
        self.mind_map_btn = ButtonFactory.create_action_button(
            "mind_map", self.input_wrap
        )
        ...
        # 添加博弈论启动/关闭按钮
        self.self_game_btn = ButtonFactory.create_action_button(
            "self_game", self.input_wrap
        )
        ...
        # 添加实时语音按钮
        self.voice_realtime_btn = ButtonFactory.create_action_button(
            "voice_realtime", self.input_wrap
        )
    ...
    def _check_active_communication(self):
        ...
        try:
            ...
            if (
                not hasattr(self, "active_comm_manager")
                or self.active_comm_manager is None
            ):
                return
        ...
        try:
            ...
            if success:
                chat.add_user_message(
                    "系统",
                    f"✅ AI日记已保存（共 {self.ai_diary_manager.get_diary_count()} 条）",
                )
            else:
    ...
    def showEvent(self, event):
        ...
        try:
            ...
            async def _send_qq_message(message: str, sender_id: str, qq_wechat_agent):
                ...
                await _send_qq_message_with_mode(
                    message,
                    sender_id,
                    qq_wechat_agent,
                    enable_voice=True,
            ...
            async def _send_qq_message_with_mode(
                message: str,
                sender_id: str,
                qq_wechat_agent,
                ...
                try:
                    logger.info(
                        f"[任务服务] 发送消息模式: reply_mode={reply_mode}, enable_voice={enable_voice}"
                    )
                    ...
                    if reply_mode == "voice" and enable_voice:
                        ...
                        await qq_wechat_agent.qq_adapter.send_message(
            ...
            async def _send_qq_voice(message: str, sender_id: str, qq_wechat_agent):
                ...
                try:
                    ...
                    if audio_data:
                        ...
                        # 发送语音
                        success = await qq_wechat_agent.qq_adapter.send_record_audio(
                            int(sender_id), audio_path
                        )
                        if success:
            ...
            async def _send_cached_qq_message(
                message: str,
                message_type: str,
            ...
            def on_active_message(message):
                ...
                if isinstance(message, ActiveMessage):
                    ...
                    original_input = (
                        message.context.get("original_input", "")
                        if message.context
                        else ""
                    )
                ...
                if msg_content and msg_content.strip():
                    # 使用信号机制，确保在主线程中更新UI
                    chat_tool.active_message_received.emit(
                        msg_content, msg_type or "suggestion", task_content
                    )
                ...
                try:
                    ...
                    if (
                        qq_wechat_agent
                        and hasattr(qq_wechat_agent, "qq_adapter")
                        and qq_wechat_agent.qq_adapter
                    ):
                        ...
                        if qq_match:
                            ...
                            logger.info(
                                f"[任务服务] 当前会话列表: {len(sessions.get('sessions', []))} 个"
                            )

                            # 安全检查：确保sessions是字典类型
                            if not isinstance(sessions, dict):
                                logger.warning(
                                    f"[任务服务] sessions类型错误: {type(sessions)}, 期望dict"
                                )
                                sender_id = None
                            else:
                                # 获取会话字典并转换为列表
                                ...
                                qq_sessions = [
                                    ...
                                    and s.get("session_id", "").startswith("qq_")
                                ]
                                logger.info(
                                    f"[任务服务] QQ会话数量: {len(qq_sessions)}, 总会话数: {len(sessions_list)}"
                                )

                            # 检查是否有新的QQ会话建立
                            current_qq_count = len(qq_sessions)
                            has_new_session = (
                                current_qq_count > config.window._last_qq_session_count
                            )

                            if qq_sessions:
                                # 使用最近的QQ会话（根据last_activity排序）
                                latest_session = max(
                                    qq_sessions, key=lambda s: s.get("last_activity", 0)
                                )
                                session_id = latest_session.get("session_id", "")
                                sender_id = session_id.replace("qq_", "")
                                logger.info(
                                    f"[任务服务] 使用最近的QQ会话: {session_id}, last_activity: {latest_session.get('last_activity', 0)}"
                                )
                                ...
                                if sender_id and sender_id.isdigit():
                                    ...
                                    if loop and loop.is_running():
                                        ...
                                        try:
                                            ...
                                            enable_voice = qq_config.get(
                                                "enable_voice", True
                                            )
                                            reply_mode = qq_config.get(
                                                "reply_mode", "both"
                                            )
                                        except Exception as e:
                                            logger.error(
                                        ...
                                        logger.info(
                                            f"[任务服务] QQ配置: enable_voice={enable_voice}, reply_mode={reply_mode}"
                                        )
    ...
    async def _send_cached_qq_messages_async(self, sender_id: str, qq_wechat_agent):
        ...
        try:
            ...
            logger.info(
                f"[缓存消息] 已完成，剩余缓存消息: {len(self._cached_qq_messages)} 条"
            )
