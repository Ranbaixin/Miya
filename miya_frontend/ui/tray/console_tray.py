...
import subprocess
import time
from PyQt5.QtWidgets import QSystemTrayIcon, QMenu, QAction, QApplication  # 统一入口 #
from PyQt5.QtCore import QTimer, pyqtSignal, QObject  # 统一入口 #
...
class ConsoleTrayManager(QObject):
    ...
    def _auto_hide_console(self):
        ...
        if self.console_hwnd and self.console_visible:
            ...
            # 显示托盘消息提示
            self.show_message("弥娅 AI", "系统已启动，控制台已隐藏到托盘")
    ...
    def _setup_tray(self):
        ...
        try:
            ...
            icon_path = os.path.join(
                os.path.dirname(__file__), "..", "img/window_icon.png"
            )
            if not os.path.exists(icon_path):
                icon_path = os.path.join(
                    os.path.dirname(__file__), "..", "img/standby.png"
                )

            # 创建系统托盘图标
            self.tray_icon = QSystemTrayIcon()
            self.tray_icon.setIcon(QIcon(icon_path))
            self.tray_icon.setToolTip("弥娅 AI - 控制台")
    ...
    def _find_console_window_windows(self):
        ...
        try:
            ...
            if self.console_hwnd:
                ...
                # 保存原始窗口样式
                self.original_style = ctypes.windll.user32.GetWindowLongW(
                    self.console_hwnd, -16
                )  # GWL_EXSTYLE
            else:
                print("未找到控制台窗口")
    ...
    def _enable_auto_start_windows(self):
        ...
        try:
            ...
            winreg.SetValueEx(key, "MiyaAI", 0, winreg.REG_SZ, command)
            winreg.CloseKey(key)
    ...
    def _enable_auto_start_macos(self):
        ...
        try:
            ...
            plist_file = os.path.join(launch_agents_dir, "com.miya.MiyaAI.plist")

            # 获取启动命令
            command = self._get_startup_command()

            plist_data = {
                "Label": "com.miya.MiyaAI",
    ...
    def _enable_auto_start_linux(self):
        ...
        try:
            ...
            desktop_file = os.path.join(autostart_dir, "miya-ai.desktop")
...
"""
    ...
    def _check_auto_start_windows(self):
        ...
        try:
            ...
            winreg.QueryValueEx(key, "MiyaAI")
            winreg.CloseKey(key)
            return True
        except:
            return False
    ...
    def show_message(
        self, title, message, icon=QSystemTrayIcon.Information, timeout=3000
    ):
        """显示托盘消息"""
        if self.tray_icon:
