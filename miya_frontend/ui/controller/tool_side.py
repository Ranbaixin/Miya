from PyQt5.QtCore import Qt, QParallelAnimationGroup, QPropertyAnimation, QEasingCurve
import os
...
class SideTool:
    ...
    def toggle_full_img(self, e):
        ...
        target_width = (
            self.expanded_width if self.full_img else self.collapsed_width
        )  # 目标宽度：展开或收缩
        ...
        # 输入框动画 - 进入设置时隐藏，退出时显示
        if self.full_img:
            input_hide_anim = QPropertyAnimation(
                window.input_wrap, b"maximumHeight", window
            )
            ...
            input_show_anim = QPropertyAnimation(
                window.input_wrap, b"maximumHeight", window
            )
    ...
    def set_fallback_image(self, image_path):
        ...
        chat.add_user_message(
            "系统", f"✅ 回退图片已设置: {os.path.basename(image_path)}"
        )
        return True
