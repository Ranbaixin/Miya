...
class VoiceTool:
    ...
    def _init_voice(self):
        ...
        if config.voice_realtime.voice_mode != "auto":
            ...
            mode_map = {
                ...
                "windows": VoiceMode.WINDOWS,
            }
            self.default_voice_mode = mode_map.get(
                config.voice_realtime.voice_mode, None
            )
        else:
            # 自动选择模式
        ...
        logger.info(
            f"[UI] 使用统一语音管理器，默认模式: {self.default_voice_mode.value if self.default_voice_mode else 'auto'}"
        )
    ...
    def start_voice_realtime(self):
        ...
        try:
            ...
            # 如果没有指定模式，使用端到端模式作为默认
            if mode is None:
                mode = VoiceMode.END_TO_END
                logger.warning("[UI] 未指定语音模式，使用默认的端到端模式")

            logger.info(
                f"[UI] 尝试启动语音服务，模式: {mode if isinstance(mode, str) else mode.value}"
            )

            # 检查API密钥要求（除了local模式）
            if mode != VoiceMode.LOCAL and not config.voice_realtime.api_key:
            ...
            if not success:
                chat.add_user_message(
                    "系统", f"❌ 语音服务启动失败（{mode.value}），请检查配置和服务状态"
                )
            else:
                # 设置激活标志
                self.voice_realtime_active = True
                logger.info("[UI] 语音服务启动成功")

        except Exception as e:
            import traceback

            logger.error(f"启动语音服务异常: {e}\n{traceback.format_exc()}")
    ...
    def toggle_voice_realtime(self):
        ...
        if not self.voice_realtime_active:
            ...
            if config.voice_realtime.provider == "local" and hasattr(
        ...
        try:
            ...
            if status == "listening":
                ...
                if not getattr(self, "_is_manual_stop", False):
                    ...
                    self.chat_tool.add_user_message(
                        "系统", "⚠️ 语音连接已断开（超时或网络问题）"
                    )
