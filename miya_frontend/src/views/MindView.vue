...
async function loadData() {
  ...
  try {
    ...
    for (const af of anchorFiles) {
      try {
        const res = await fetch(`http://localhost:${Number(import.meta.env.VITE_API_PORT) || 9800}/api/desktop/files/read?path=${encodeURIComponent(af.path)}`).then(r => r.json())
        if (res?.lines) {
    ...
    for (const d of dirs) {
      try {
        const listRes = await fetch(`http://localhost:${Number(import.meta.env.VITE_API_PORT) || 9800}/api/desktop/files/list?path=${encodeURIComponent(d.dir)}`).then(r => r.json())
        ...
        for (let batch = 0; batch < shuffled.length; batch += 10) {
          ...
          const results = await Promise.allSettled(batchFiles.map((f: any) =>
            fetch(`http://localhost:${Number(import.meta.env.VITE_API_PORT) || 9800}/api/desktop/files/read?path=${encodeURIComponent(`${d.dir}/${f.name}`)}`).then(r => r.json())
          ))
...
async function search() {
  ...
  try {
    ...
    for (const path of scanFiles) {
      try {
        const res = await fetch(`http://localhost:${Number(import.meta.env.VITE_API_PORT) || 9800}/api/desktop/files/read?path=${encodeURIComponent(path)}`).then(r => r.json())
        if (res?.lines) {
          const arr = JSON.parse(res.lines.join(''))
          for (const item of (Array.isArray(arr) ? arr : [arr])) {
