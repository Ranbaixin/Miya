...
from miya_psyarch.core.state_pool.energy_view import build_energy_flow_trace
from miya_psyarch.core.state_pool.energy_updater import PredictionEnergyUpdater
from miya_psyarch.core.state_pool.residual_tracker import ResidualTracker
...
class DualEnergyStatePool:
    ...
    def apply_external_items(self, items: list[dict], *, tick_index: int) -> None:
        ...
        for item in items:
            ...
            if isinstance(item.get("anchor_meta"), dict):
                entry.anchor_meta.update(dict(item.get("anchor_meta", {}) or {}))
    ...
    def read_r_state(self, *, items_per_head: int | None = None, head_limit: int | None = None) -> dict:
        ...
        def _row_for_label(label: str) -> dict | None:
            ...
            row["query_weight"] = (
                max(0.0, row["real_energy"] * 1.0 + row["virtual_energy"] * 0.65 + row["attention_gain"] * 0.8 - row["fatigue"] * 0.3)
            )
            row["attention_score"] = (
                max(0.0, row["cognitive_pressure"] * 0.7 + row["attention_gain"] - row["fatigue"] * 0.5 + row["virtual_energy"] * 0.25)
            )
            row_cache[label] = row
            return row
        ...
        # head_recent: external evidence first (bounded).
        recent_labels = _unique_labels(list(reversed(list(self._recent_external)[-max(1, self.recent_external_limit) :])))
        # IMPORTANT:
    ...
    def snapshot_for_memory_write(self) -> dict:
        ...
        - Force-include all "external evidence" items that were observed at this tick.
          (Currently: `source_type` in {external_text, vision_bridge*, audio_bridge*} and
