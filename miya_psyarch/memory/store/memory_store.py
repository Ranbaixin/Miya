...
from miya_psyarch.memory.embedding.online_store import OnlineEmbeddingStore
from miya_psyarch.memory.persistence import MemoryPersistenceAdapter, NullMemoryPersistence
from miya_psyarch.memory.relations import RelativeRelationStore
from miya_psyarch.memory.retrieval.faiss_index import FaissHnswConfig, FaissHnswIndex
from miya_psyarch.memory.retrieval.hash_vector_index import HashVectorIndex
from miya_psyarch.memory.retrieval.numeric_feature_index import NumericFeatureIndex, NumericFeatureIndexConfig
from miya_psyarch.memory.retrieval.posting_index import PostingIndex
from miya_psyarch.memory.spacetime.transition_store import TransitionStore
...
class MemoryStore:
    def __init__(
        ...
        *,
        ...
        temporal_fatigue_window_ticks: int = 10,
        temporal_fatigue_strength: float = 0.08,
        ...
        self.scoring_candidate_limit = max(8, int(scoring_candidate_limit))
        ...
        self.temporal_fatigue_strength = _clamp(float(temporal_fatigue_strength), 0.0, 0.45)
    ...
    def write_snapshot(self, *, tick_index: int, memory_kind: str, items: list[dict], focus_labels: list[str], source_text: str, asset_refs: list[dict] | None = None) -> dict:
        self._online.begin_tick(int(tick_index))
        ...
        for item in items:
            if not isinstance(item, dict):
                continue
            if "tick_index" in item:
                stamped = item
            else:
                stamped = dict(item)
                stamped["tick_index"] = int(tick_index)
        ...
        self._queue_index_job(snapshot=snapshot, features=snapshot_features, vector=vector, previous=previous)
        if snapshot["memory_kind"] == "focus" or len(snapshot.get("items", []) or []) <= 64:
            immediate = self._process_index_job_by_id(snapshot["memory_id"])
            self._cache_stats["index_job_processed"] += int(immediate.get("processed", 0) or 0)
            self._cache_stats["index_job_skip_missing"] += int(immediate.get("skipped", 0) or 0)
    ...
    def persistence_summary(self) -> dict:
        ...
        return {
            **dict(adapter_summary or {}),
            ...
            "policy": {
                "authoritative_layer": "PostgreSQL/pgvector when adapter is attached",
    ...
    def ingest_persisted_snapshots(
        ...
        *,
        ...
        from raw snapshots can double-count old experience. P1-H-3 should restore
        online learning from persisted learning events or compacted summaries.
        """
        ...
        for row in rows:
            ...
            self._append_loaded_snapshot(
                ...
                previous=previous_by_kind.get(memory_kind),
                process_indexes=process_indexes,
                replay_learning=replay_learning,
            )
    ...
    def recall(self, query_items: list[dict], *, memory_kind: str, top_k: int | None = None, time_context: dict | None = None) -> list[dict]:
        ...
        for candidate_index, candidate in enumerate(merged[: self.scoring_candidate_limit]):
            ...
            energy_overlap = self._energy_overlap(query_energy, snapshot_energy, query_mass=query_mass, snapshot_mass=snapshot_mass)
    ...
    def successors(
        ...
        *,
        ...
        cache_epoch = int(self._memory_revision)
        cache_key = (cache_epoch, str(memory_kind or ""), str(memory_id or ""), int(top_limit))
    ...
    def _scale_successor_rows_by_b(self, rows: list[dict], *, source_b_row: dict | None) -> list[dict]:
        ...
        for row, preliminary_weight, successor_weight, calibration in zip(clean_rows, preliminary_weights, weights, calibrations):
            transfer_mass = self._prediction_energy_transfer_mass(source_mass * successor_weight)
    ...
    def _append_loaded_snapshot(
        ...
        *,
        ...
        self._queue_index_job(snapshot=snapshot, features=features, vector=vector, previous=previous)
        if process_indexes:
    ...
    def _queue_index_job(self, *, snapshot: dict, features: dict, vector: list[float], previous: dict | None) -> None:
        memory_id = str(snapshot.get("memory_id", "") or "")
        if not memory_id:
            return
    ...
    def _get_or_build_candidates(self, *, memory_kind: str, query_signature: str, query_features: dict) -> tuple[list[dict], list[dict], list[dict]]:
        ...
        if cached is not None:
            ...
            return posting_rows, list(cached.get("vector_rows", []) or []), numeric_rows
        ...
        return posting_rows, list(vector_rows), numeric_rows
    ...
    def _energy_overlap(
        ...
        *,
        ...
        for label, value in smaller.items():
            ...
            if other is None:
                continue
            numerator += min(float(value or 0.0), float(other or 0.0))
    ...
    def _current_tick_for_temporal(self, query_items: list[dict], *, time_context: dict | None = None) -> int | None:
        if time_context and time_context.get("current_tick") is not None:
            try:
                return int(float(time_context.get("current_tick")))
            except (TypeError, ValueError):
                return None
        ticks: list[int] = []
    ...
    def _temporal_applicability(self, snapshot: dict | None, *, current_tick: int | None) -> dict:
        ...
        return {
            "weight": _round4(_clamp(weight, self.temporal_floor, 1.0 + self.temporal_recent_gain)),
    ...
    def _build_prediction_payload_items(self, snapshot: dict, *, limit: int | None = None) -> list[dict]:
        ...
        ordered = list(external_now) + list(outcome_now[:4]) + list(action_now[:4]) + list(rest)
        return [dict(item) for item in ordered[:cap] if isinstance(item, dict)]
    ...
    def _item_state_field_weight(self, item: dict) -> float:
        real = max(0.0, float(item.get("real_energy", 0.0) or 0.0))
        virtual = max(0.0, float(item.get("virtual_energy", 0.0) or 0.0))
        pressure = abs(float(item.get("cognitive_pressure", real - virtual) or 0.0))
        ...
        return _round4(real + virtual * 0.65 + pressure * 0.25 + query * 0.55 + attention * 0.35 + focus_bonus)
