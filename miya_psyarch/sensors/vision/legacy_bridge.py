...
try:
    ...
    VisionSensorV1 = None  # type: ignore[assignment]
...
class LegacyVisionBridge:
    def __init__(self) -> None:
        self._available = VisionSensorV1 is not None
        if self._available:
            self.sensor = VisionSensorV1(patch_budget=24, focus_patch_budget=12)  # type: ignore[misc]

    def ingest_image_bytes(self, raw_bytes: bytes, *, tick_index: int, source_type: str = "image_input") -> dict:
        if not self._available:
            return {"packet": {}, "state_items": [], "inner_vision": {}}
        ...
        inner_view = {
            ...
            "energy_summary": {
                ...
                "dynamic_track_count": int(
                    ((packet.get("dynamic_track_summary", {}) or {}).get("track_count", 0) or 0)
                ),
                "dynamic_object_count": int(
                    ((packet.get("dynamic_track_summary", {}) or {}).get("object_count", 0) or 0)
                ),
            },
