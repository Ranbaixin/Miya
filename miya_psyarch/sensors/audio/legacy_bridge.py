...
try:
    ...
    HearingSensorV1 = None  # type: ignore[assignment]
...
class LegacyAudioBridge:
    def __init__(self) -> None:
        self._available = HearingSensorV1 is not None
        if self._available:
            self.sensor = HearingSensorV1(window_budget=12)  # type: ignore[misc]

    def ingest_wav_bytes(self, raw_bytes: bytes, *, tick_index: int, source_type: str = "audio_input") -> dict:
        if not self._available:
            return {"packet": {}, "state_items": [], "inner_audio": {}}
        ...
        inner_view = {
            "current_bands": [
                ...
                for item in (packet.get("windows", []) or [])[:8]
                if str(item.get("sa_label", "") or "")
            ],
            "primary_peaks": [
                str(item.get("sa_label", "") or "")
                for item in (packet.get("focus_priority_samples", []) or [])[:8]
                if str(item.get("sa_label", "") or "")
            ],
            "recall_stream": [],
            "prediction_stream": [],
            ...
            "energy_summary": {
                ...
                "dominant_hz": _round4(
                    float(((packet.get("feature_summary", {}) or {}).get("dominant_hz", 0.0) or 0.0))
                ),
                "tonal_clarity": _round4(
                    float(((packet.get("feature_summary", {}) or {}).get("tonal_clarity", 0.0) or 0.0))
                ),
            },
        }
        return {
