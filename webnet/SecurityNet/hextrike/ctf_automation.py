...
try:
    ...
    class ModernVisualEngine:
        @staticmethod
        def format_tool_status(tool, status, message=""):
            return f"[{tool}] {status}: {message}"

        @staticmethod
        def format_error_card(level, tool, message, extra=""):
            return f"[{level}] {tool}: {message}"

        @staticmethod
        def format_highlighted_text(text, color=""):
            return text

        @staticmethod
        def format_command_execution(cmd, status=""):
            return f"$ {cmd} [{status}]"

        @staticmethod
        def format_success_message(msg):
            return msg

        @staticmethod
        def format_warning_message(msg):
            return msg
