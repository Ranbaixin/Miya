        ...
        try:
            ...
            if context.memory_engine:
                ...
                for memory_id, memory_data in all_memories.items():
                    ...
                    if isinstance(memory_data, dict):
                        task_info = {"id": memory_id.replace("scheduled_task_", ""), "data": memory_data}
                    ...
                    for task in task_queue:
                        ...
                        if task_id and task_id not in [t["id"] for t in tasks]:
                            tasks.append(
                                {
                                    "id": task_id,
                                    "data": {"task_type": getattr(task, "task_type", "unknown"), "status": "pending"},
                                }
                            )
                    ...
                    for task_id, task in running_tasks.items():
                        if task_id not in [t["id"] for t in tasks]:
                            tasks.append(
                                {
                                    "id": task_id,
                                    "data": {"task_type": getattr(task, "task_type", "unknown"), "status": "running"},
                                }
                            )
                        else:
                            # 更新状态
                    ...
                    for task_id, task in completed_tasks.items():
                        if task_id not in [t["id"] for t in tasks]:
                            tasks.append(
                                {
                                    "id": task_id,
                                    "data": {"task_type": getattr(task, "task_type", "unknown"), "status": "completed"},
                                }
                            )
                        else:
                            # 更新状态
            ...
            if not tasks:
                ...
                if scheduler and hasattr(scheduler, "task_store") and scheduler.task_store:
                    db_tasks = scheduler.task_store.list_tasks(status=status_filter if status_filter != "all" else None)
                    for td in db_tasks:
                        tasks.append({"id": td["task_id"], "data": td})
            ...
            for i, task in enumerate(tasks, 1):
                ...
                # 状态图标
                status_icon = {"pending": "⏳", "running": "▶️", "completed": "✅", "failed": "❌"}.get(
                    task_status, "❓"
                )
