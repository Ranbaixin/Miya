...
Rust 音频引擎是有意设计为实时渲染器，而非数据源。
本模块将可编辑的 DAW 项目以 JSON 形式持久化，
使 WebUI、Agent 工具、宿主同步路径均基于同一数据模型。
"""
...
# ─── normalize_project (全量规格化，约 200 行) ────────────────────
...
def normalize_project(project: dict[str, Any] | None) -> dict[str, Any]:
    ...
    normalized: dict[str, Any] = {
        ...
        "automation_learned_parameters": _normalize_learned_parameters(project.get("automation_learned_parameters")),
        "tracks": [],
    }
    ...
    for index, raw_track in enumerate(raw_tracks):
        ...
        declared_type = str(raw_track.get("type", raw_track.get("track_type", "")) or "").strip().lower()
        if declared_type == "automation":
            ...
            legacy_notes = [_normalize_note(note) for note in raw_track.get("notes", []) if isinstance(note, dict)]
    ...
    max_clip_end = max(
        (clip["start"] + clip["duration"] for track in normalized["tracks"] for clip in track["clips"]),
        default=0.0,
    )
...
def delete_track(track_id: int) -> tuple[dict[str, Any], dict[str, Any]]:
    ...
    project["tracks"] = [item for item in project["tracks"] if int(item.get("id", -1)) != deleted_id]
    project = save_project(project)
        ...
        "tracks": [
            {
                ...
                "audio_clips": sum(
                    1 for clip in track.get("clips", []) if isinstance(clip, dict) and clip.get("type") == "audio"
                ),
...
# ─── MIDI 操作 (midi_write, midi_diff, midi_batch_edit, midi_query, midi_inspect) ──
...
def midi_diff(
    ...
    for op in operations:
        ...
        if op_type == "add_note":
            ...
            clip["events"].append(_normalize_midi_event(_event_payload_to_clip_local(payload, clip)))
            ...
            added, deleted = _apply_midi_event_curve(clip, _curve_op_to_clip_local(op, clip), op_type)
...
def midi_batch_edit(
    ...
    *,
    ...
    for raw_op in operations:
        ...
        op = dict(raw_op)
        op_type = str(op.get("op") or op.get("type") or "").strip().lower()
        op_selection = _normalize_selection(project, op.get("selection"), base=base_selection, op=op)
        ...
        if op_type in {
            ...
            changed["events_deleted"] += deleted
            changed["curves_written"] += 1
            changed["details"].append({"op": op_type, "events_added": added, "events_deleted": deleted})
...
def midi_query(
    *,
    ...
    if "clips" in include_set:
        response["clips"] = [
            _clip_query_summary(track, clip) for track, clip in _selected_midi_clips(project, normalized_selection)
        ]
...
def midi_inspect(
    *,
    ...
    if "events" in include_set or "midi_events" in include_set:
        rows.extend(_event_detail(ref) for ref in _selected_event_refs(project, normalized_selection))
    rows.sort(key=lambda row: (float(row.get("start", 0.0)), row.get("kind", ""), row.get("id", "")))
...
def _normalize_clips(
    ...
    *,
    ...
    if isinstance(raw_clips, list):
        clips = [_normalize_clip(clip, track_color=track_color) for clip in raw_clips if isinstance(clip, dict)]
...
def _normalize_clip(clip: dict[str, Any], *, track_color: str) -> dict[str, Any]:
    ...
    if clip_type not in {"midi", "audio"}:
        clip_type = "midi"
    notes = [_normalize_note(note) for note in clip.get("notes", []) if clip_type == "midi" and isinstance(note, dict)]
...
def _normalize_meter(value: Any) -> list[int]:
    if isinstance(value, (list, tuple)) and len(value) == 2:
        ...
        den = value[1] if int(value[1]) in METER_DENOMINATORS else 4
        return [num, den]
    return [4, 4]
...
def _normalize_plugin_slots(track: dict[str, Any], *, track_type: str = "instrument") -> list[dict[str, Any]]:
    ...
    if not any(slot.get("id") == "instrument" for slot in slots):
        slots.insert(
            ...
            _normalize_plugin_slot(
                {"type": "builtin", "name": track.get("instrument") or "ATRI Basic Synth"},
                slot_id="instrument",
            ),
        )
...
def _normalize_plugin_slot(plugin: dict[str, Any] | None, *, slot_id: str = "instrument") -> dict[str, Any]:
...
def _normalize_automation_target(value: Any) -> dict[str, Any]:
    ...
    if kind not in {*TRACK_AUTOMATION_TARGET_KINDS, *GLOBAL_AUTOMATION_TARGET_KINDS, "unassigned"}:
        kind = "track_volume"
...
def _normalize_automation_point(value: dict[str, Any], *, target: dict[str, Any]) -> dict[str, Any]:
    ...
    return {"id": point_id, "beat": round(beat, 6), "value": point_value, "curve": curve}
...
def _slot_label(slot_id: Any) -> str:
    ...
    if slot.startswith("insert_"):
        return f"Insert {slot.removeprefix('insert_')}"
    return slot
...
def _normalize_selection(
    ...
    *,
    ...
    if controllers:
        raw["controllers"] = sorted(set(_bounded_int(v, 0, 0, 127) for v in controllers))

    event_types = [_normalize_midi_event_type(v) for v in _as_str_list(raw.get("event_types"))]
...
def _validate_midi_batch_write_scope(
    ...
    *,
    ...
    has_track_scope = track_id is not None or bool(raw_selection.get("track_ids") or raw_selection.get("track_id"))
...
def _selected_midi_clips(
    ...
    *,
    ...
    for track in _selected_tracks(project, selection):
        clips = [clip for clip in track.get("clips", []) if isinstance(clip, dict) and clip.get("type") == "midi"]
...
def _selected_note_refs(project: dict[str, Any], selection: dict[str, Any]) -> list[dict[str, Any]]:
    ...
    for track, clip in _selected_midi_clips(project, selection):
        ...
        for note in clip.get("notes", []):
            ...
            if note_ids and str(note.get("id")) not in note_ids:
                continue
            if pitch_range and not (int(pitch_range[0]) <= int(note["pitch"]) <= int(pitch_range[1])):
                continue
    ...
    return sorted(refs, key=lambda ref: (ref["absolute_start"], ref["note"]["pitch"], ref["note"]["id"]))
...
def _selected_event_refs(
    ...
    for track, clip in _selected_midi_clips(project, selection):
        ...
        for event in clip.get("events", []):
            ...
            if beat_range and not (beat_range[0] - 1e-6 <= absolute_start <= beat_range[1] + 1e-6):
                continue
            refs.append({"track": track, "clip": clip, "event": event, "absolute_start": absolute_start})
    return sorted(refs, key=lambda ref: _midi_event_sort_key({**ref["event"], "start": ref["absolute_start"]}))
...
def _clip_query_summary(track: dict[str, Any], clip: dict[str, Any]) -> dict[str, Any]:
    ...
    return {
        ...
        "lanes": _event_lane_summaries([{"event": ev, "absolute_start": clip["start"] + ev["start"]} for ev in events]),
    }
...
def _event_lane_summaries(refs: list[dict[str, Any]]) -> list[dict[str, Any]]:
    ...
    for ref in refs:
        ...
        ev_val = _event_numeric_value(event)
        if ev_val is not None:
            lane["values"].append(ev_val)
    summaries = []
...
# ─── Timeline Write / Clip Helpers ───────────────────────────────


def _track_midi_clips(track: dict[str, Any]) -> list[dict[str, Any]]:
    return [clip for clip in track.get("clips", []) if isinstance(clip, dict) and clip.get("type") == "midi"]
...
def _find_timeline_note(track: dict[str, Any], op: dict[str, Any]) -> dict[str, Any] | None:
    for clip in _track_midi_clips(track):
        for note in clip.get("notes", []):
...
def _find_timeline_event(track: dict[str, Any], op: dict[str, Any]) -> dict[str, Any] | None:
    for clip in _track_midi_clips(track):
        for event in clip.get("events", []):
...
def _absolute_to_clip_local(absolute: float, clip: dict[str, Any]) -> float:
    ...
    if absolute < clip_start - 1e-6:
        raise ValueError(f"absolute beat {absolute:g} is before MIDI clip {clip.get('id')} start {clip_start:g}")
...
def _apply_midi_event_curve(
    ...
    for beat, value in sampled_points:
        event = {**target, "id": f"e_{uuid4().hex[:10]}", "start": beat, value_field: value}
    ...
    if isinstance(raw_points, list) and raw_points:
        for raw_point in raw_points:
            if isinstance(raw_point, dict):
                ...
                continue
            points.append((_non_negative_float(beat, 0.0), _bounded_int(value, default, minimum, maximum)))
    if not points:
        if any(key in op for key in ("start_value", "end_value", "from", "to")):
            ...
            points = [
                (_non_negative_float(op.get("start"), 0.0), _bounded_int(start_value, default, minimum, maximum)),
                (_non_negative_float(op.get("end"), 0.0), _bounded_int(end_value, default, minimum, maximum)),
            ]
        elif "value" in op or value_field in op:
            value = _first_present(op, (value_field, "value"), default=default)
            points = [(_non_negative_float(op.get("start"), 0.0), _bounded_int(value, default, minimum, maximum))]
    if not points:
...
def _sample_beats_with_limit(start: float, end: float, resolution: float) -> list[float]:
    ...
    if estimated_points > MIDI_CURVE_MAX_POINTS:
        raise ValueError(
            f"MIDI curve would generate too many points "
...
# ─── Batch Velocity / Event Curve Operations ────────────────────
...
def _apply_batch_event_curve_operation(
    ...
    explicit_points = _batch_explicit_curve_points(event_op, value_field, minimum, maximum, default)
    explicit_range = _explicit_points_range(explicit_points)
    split_across = len(clips) > 1 and not selection.get("clip_ids")
    added = 0
    deleted = 0
    for _track, clip in clips:
        ...
        if absolute_range:
            ...
            abs_end = min(absolute_range[1], clip_end) if split_across else absolute_range[1]
        elif explicit_range:
            abs_start = max(explicit_range[0], clip_start)
            abs_end = min(explicit_range[1], clip_end) if split_across else explicit_range[1]
        else:
...
def _apply_batch_event_clear(project: dict[str, Any], selection: dict[str, Any], op: dict[str, Any]) -> int:
    ...
    for _track, clip in _selected_midi_clips(project, selection):
        ...
        if not event_ids:
            continue
        before = len(clip.get("events", []))
        clip["events"] = [event for event in clip["events"] if str(event.get("id")) not in event_ids]
        deleted += before - len(clip["events"])
    return deleted
...
def _batch_curve_points_for_range(
    ...
    *,
    ...
    if explicit_points is not None:
        in_range = [(b, v) for b, v in explicit_points if target_start - 1e-6 <= b <= target_end + 1e-6]
        if resolution is None:
            return in_range
        beats = _sample_beats_with_limit(target_start, target_end, resolution)
...
def _operation_beat_range(selection: dict[str, Any], refs: list[dict[str, Any]]) -> tuple[float, float]:
...
def _accent_matches(beat: float, op: dict[str, Any]) -> bool:
    ...
    if pattern == "backbeat":
        return _beat_mod_matches(beat, 4.0, 1.0, tolerance) or _beat_mod_matches(beat, 4.0, 3.0, tolerance)
...
def _shape_value(
    ...
    *,
    ...
    low = _bounded_int(
        _first_present(op, ("min", "minimum", "low"), default=default_min), default_min, minimum, maximum
    )
    high = _bounded_int(
        _first_present(op, ("max", "maximum", "high"), default=default_max), default_max, minimum, maximum
    )
    ...
    elif shape in {"crescendo", "fade_in", "linear", "ramp"} and start_value is None and end_value is None:
    ...
    if shape in {"swell", "phrase_swell"}:
        ...
        value = _interpolate_scalar(
            start_value if start_value is not None else low, end_value if end_value is not None else high, unit * unit
        )
    elif shape == "ease_out":
        value = _interpolate_scalar(
        ...
        value = _interpolate_scalar(
            start_value if start_value is not None else low, end_value if end_value is not None else high, unit
        )
...
def _update_midi_clip_duration(clip: dict[str, Any]) -> None:
    ...
    event_end = max(
        (float(event.get("start", 0.0) or 0.0) for event in clip.get("events", []) if isinstance(event, dict)),
        default=0.0,
    )
