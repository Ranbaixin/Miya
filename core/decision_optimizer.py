...
from core.autonomous_engine import AutonomousEngine, DecisionType, RiskLevel, get_autonomous_engine
...
class DecisionOptimizer:
    ...
    def analyze_decisions(self) -> List[DecisionPattern]:
        ...
        self.metrics["high_confidence_patterns"] = sum(
            1 for p in patterns if p.confidence >= self.min_confidence_threshold
        )
    ...
    def get_recommendation(self, problem_type: str, severity: str, file_path: Optional[str]) -> Optional[DecisionType]:
        ...
        for pattern in self.patterns:
            if (
                pattern.problem_type == problem_type
                and pattern.severity == severity
                and pattern.file_pattern == file_pattern
