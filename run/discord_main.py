...
使用方法:
  python run/discord_main.py

配置文件: config/platforms_config.py 中的 DISCORD_CONFIG
"""
...
from config.platforms_config import DISCORD_CONFIG
...
logging.basicConfig(
    ...
    handlers=[
        ...
        logging.FileHandler(
            project_root / "logs" / "miya_discord.log", encoding="utf-8"
        ),
    ],
)
logger = logging.getLogger("MiyaDiscord")


class MiyaDiscord:
    """弥娅 Discord 机器人 - 完整集成弥娅系统"""

    def __init__(self):
        self.settings = Settings()
        self.miya = None
        self.token = DISCORD_CONFIG.get("bot_token", "")
        self.super_admin = os.getenv("DISCORD_SUPER_ADMIN", "")

    async def initialize(self):
        """初始化弥娅系统"""
        logger.info("=" * 50)
        logger.info("弥娅 Discord 机器人初始化中...")
        logger.info("=" * 50)
...
╚════════════════════════════════════════════════════════════╝
    ...
    miya_bot = MiyaDiscord()

    if not miya_bot.token:
        logger.error("❌ 请在 config/platforms_config.py 中配置 Discord bot_token")
        return
    ...
    try:
        ...
        await client.start(miya_bot.token)
