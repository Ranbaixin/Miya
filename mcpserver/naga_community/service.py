...
class NagaCommunityService:
    ...
    async def _login(self, call: Dict[str, Any]) -> str:
        username = call.get("username", "")
        password = call.get("password", "")
        if not username or not password:
            return json.dumps(
                {"error": "缺少 username 或 password"}, ensure_ascii=False
            )
        result = await self._auth.login(str(username), str(password))
        return json.dumps(result, ensure_ascii=False)

    async def _register(self, call: Dict[str, Any]) -> str:
        username = call.get("username", "")
    ...
    async def _send_verification(self, call: Dict[str, Any]) -> str:
        email = call.get("email", "")
        username = call.get("username", "")
        if not email or not username:
            return json.dumps({"error": "缺少 email 或 username"}, ensure_ascii=False)
        result = await self._auth.send_verification_code(str(email), str(username))
        return json.dumps(result, ensure_ascii=False)

    async def _logout(self, call: Dict[str, Any]) -> str:
        result = await self._auth.logout()
        return json.dumps(result, ensure_ascii=False)

    async def _get_me(self, call: Dict[str, Any]) -> str:
        if not self._auth.is_logged_in:
            return json.dumps({"error": "未登录，请先调用 login"}, ensure_ascii=False)
        result = await self._auth.get_me()
        return json.dumps(result, ensure_ascii=False)

    # ===== 帖子实现 =====

    async def _get_posts(self, call: Dict[str, Any]) -> str:
    ...
    async def _get_post_detail(self, call: Dict[str, Any]) -> str:
        post_id = call.get("post_id", "")
        if not post_id:
            return json.dumps({"error": "缺少 post_id"}, ensure_ascii=False)
        result = await self._forum.get_post_detail(str(post_id))
        return json.dumps(result, ensure_ascii=False)

    async def _create_post(self, call: Dict[str, Any]) -> str:
        if not self._auth.is_logged_in:
            return json.dumps({"error": "未登录"}, ensure_ascii=False)
        title = call.get("title", "")
        content = call.get("content", "")
        if not title or not content:
            return json.dumps({"error": "缺少 title 或 content"}, ensure_ascii=False)
        tags = call.get("tags", "")
    ...
    async def _delete_post(self, call: Dict[str, Any]) -> str:
        if not self._auth.is_logged_in:
            return json.dumps({"error": "未登录"}, ensure_ascii=False)
        post_id = call.get("post_id", "")
        if not post_id:
    ...
    async def _like_post(self, call: Dict[str, Any]) -> str:
        if not self._auth.is_logged_in:
            return json.dumps({"error": "未登录"}, ensure_ascii=False)
        post_id = call.get("post_id", "")
        if not post_id:
    ...
    async def _comment_post(self, call: Dict[str, Any]) -> str:
        if not self._auth.is_logged_in:
            return json.dumps({"error": "未登录"}, ensure_ascii=False)
        post_id = call.get("post_id", "")
        content = call.get("content", "")
        if not post_id or not content:
            return json.dumps({"error": "缺少 post_id 或 content"}, ensure_ascii=False)
    ...
    async def _like_comment(self, call: Dict[str, Any]) -> str:
        if not self._auth.is_logged_in:
            return json.dumps({"error": "未登录"}, ensure_ascii=False)
        comment_id = call.get("comment_id", "")
        if not comment_id:
            return json.dumps({"error": "缺少 comment_id"}, ensure_ascii=False)
        result = await self._forum.like_comment(str(comment_id))
        return json.dumps(result, ensure_ascii=False)

    async def _delete_comment(self, call: Dict[str, Any]) -> str:
        if not self._auth.is_logged_in:
            return json.dumps({"error": "未登录"}, ensure_ascii=False)
        comment_id = call.get("comment_id", "")
        if not comment_id:
    ...
    async def _get_messages(self, call: Dict[str, Any]) -> str:
        if not self._auth.is_logged_in:
            return json.dumps({"error": "未登录"}, ensure_ascii=False)
        page = int(call.get("page", 1))
    ...
    async def _send_message(self, call: Dict[str, Any]) -> str:
        if not self._auth.is_logged_in:
            return json.dumps({"error": "未登录"}, ensure_ascii=False)
        ...
        if not to_user_id or not content:
            return json.dumps(
                {"error": "缺少 to_user_id 或 content"}, ensure_ascii=False
            )
        post_id = call.get("post_id")
    ...
    async def _get_friend_requests(self, call: Dict[str, Any]) -> str:
        if not self._auth.is_logged_in:
            return json.dumps({"error": "未登录"}, ensure_ascii=False)
        result = await self._forum.get_friend_requests()
        return json.dumps(result, ensure_ascii=False)

    async def _accept_friend(self, call: Dict[str, Any]) -> str:
        if not self._auth.is_logged_in:
            return json.dumps({"error": "未登录"}, ensure_ascii=False)
        request_id = call.get("request_id", "")
        if not request_id:
            return json.dumps({"error": "缺少 request_id"}, ensure_ascii=False)
        result = await self._forum.accept_friend(str(request_id))
        return json.dumps(result, ensure_ascii=False)

    async def _decline_friend(self, call: Dict[str, Any]) -> str:
        if not self._auth.is_logged_in:
            return json.dumps({"error": "未登录"}, ensure_ascii=False)
        request_id = call.get("request_id", "")
        if not request_id:
    ...
    async def _get_connections(self, call: Dict[str, Any]) -> str:
        if not self._auth.is_logged_in:
            return json.dumps({"error": "未登录"}, ensure_ascii=False)
    ...
    async def _get_notifications(self, call: Dict[str, Any]) -> str:
        if not self._auth.is_logged_in:
            return json.dumps({"error": "未登录"}, ensure_ascii=False)
        result = await self._forum.get_notifications()
        return json.dumps(result, ensure_ascii=False)

    async def _read_notification(self, call: Dict[str, Any]) -> str:
        if not self._auth.is_logged_in:
            return json.dumps({"error": "未登录"}, ensure_ascii=False)
        notif_id = call.get("notification_id", "")
        if not notif_id:
            return json.dumps({"error": "缺少 notification_id"}, ensure_ascii=False)
        result = await self._forum.read_notification(str(notif_id))
        return json.dumps(result, ensure_ascii=False)

    # ===== 档案 =====

    async def _get_profile(self, call: Dict[str, Any]) -> str:
        if not self._auth.is_logged_in:
            return json.dumps({"error": "未登录"}, ensure_ascii=False)
